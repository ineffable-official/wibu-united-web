...
export default function AddStatus() {
  ...
  const screenClick = (evt) => {
    if (evt.target.id === "screen-view") navigate("/dashboard/status");
  };
  ...
  const onSubmit = (event) => {
    ...
    axios
      .post(apiUrl + "/status", form, {
        headers: {
          Authorization: `Bearer ${apiKey}`,
        },
      })
      .then((res) => {
        if (res.status === 200) navigate("/dashboard/status");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    ...
    >
      <div
        ...
        id="form-container"
      >
        <h1 className="font-bold text-xl">Add Status</h1>
