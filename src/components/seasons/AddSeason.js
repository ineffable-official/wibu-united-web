...
export default function AddSeason() {
  ...
  const screenClick = (evt) => {
    if (evt.target.id === "screen-view") navigate("/dashboard/seasons");
  };
  ...
  const onSubmit = (event) => {
    ...
    axios
      .post(apiUrl + "/seasons", form, {
        headers: {
          Authorization: `Bearer ${apiKey}`,
        },
      })
      .then((res) => {
        if (res.status === 200) navigate("/dashboard/seasons");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    ...
    >
      <div
        ...
        id="form-container"
      >
        <h1 className="font-bold text-xl">Add Season</h1>
